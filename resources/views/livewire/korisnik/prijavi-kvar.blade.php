<div class="p-6">
    ...
    <div class="bg-green-100 border-t-4 border-green-500 rounded-b text-teal-900 px-4 py-2 shadow-md" role="alert">
        <div class="flex">
            <div class="py-1"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-6 w-6 text-green-500 mr-4"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>
            </div>
            <div>
            <p class="font-bold mt-1">{{session('status')}}</p>
            </div>
        </div>
    </div>
    @endif
                    ...
                    <table class="min-w-full divide-y divide-gray-200" style="width: 100% !important">
                        <thead>
                            <tr>
                                ...
                                <th class="px-2 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6"><path stroke-linecap="round" stroke-linejoin="round" d="M20.25 8.511c.884.284 1.5 1.128 1.5 2.097v4.286c0 1.136-.847 2.1-1.98 2.193-.34.027-.68.052-1.02.072v3.091l-3-3c-1.354 0-2.694-.055-4.02-.163a2.115 2.115 0 0 1-.825-.242m9.345-8.334a2.126 2.126 0 0 0-.476-.095 48.64 48.64 0 0 0-8.048 0c-1.131.094-1.976 1.057-1.976 2.192v4.286c0 .837.46 1.58 1.155 1.951m9.345-8.334V6.637c0-1.621-1.152-3.026-2.76-3.235A48.455 48.455 0 0 0 11.25 3c-2.115 0-4.198.137-6.24.402-1.608.209-2.76 1.614-2.76 3.235v6.226c0 1.621 1.152 3.026 2.76 3.235.577.075 1.157.14 1.74.194V21l4.155-4.155" /></svg></th>
                            ...
                            @if (true)
                                @foreach ($data as $item)
                                    <tr class="@if($item->tstid == 3) bg-emerald-50 @endif">
                                        ...
                                        <td class="px-2 pr-2">{{ App\Http\TimeFormatHelper::datumFormat($item->crt)}}</td>
                                        <td class="px-2 pr-2">
                                        ...
                                        <td class="px-2 pr-2 max-w-64"><span class="px-2">{{$item->opis}} </span></td>
                                        <td class="px-2 pr-2"><span class="font-bold text-{{$item->prioritet_txt_collor}}">{{ $item->prioritet_naziv }}</span></td>
                                        <td>2</td>                           
                                        <td class="px-2 pr-2">
                                        <a href="#" class="my-1 bg-neutral-400 hover:bg-neutral-600 text-white font-bold py-1 px-2 border border-neutral-600 rounded flex">
